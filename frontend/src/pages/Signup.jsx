...
import { Mail, Lock, User, ChevronRight, ChevronLeft, CheckCircle2, Eye, EyeOff, Sparkles } from "lucide-react";
...
const Field = ({ icon: Icon, label, type, value, onChange, placeholder, required }) => {
  const [show, setShow] = useState(false);
  const isPassword = type === "password";
  return (
    <div>
      <label className="block text-slate-400 text-xs font-medium mb-1.5 uppercase tracking-widest">{label}</label>
      <div className="relative">
        ...
        />
        <Icon className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-600 w-4 h-4 pointer-events-none z-10" />
        {isPassword && (
          ...
          {step === 1 ? (
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
              ...
              <h2 className="text-3xl font-bold text-white leading-tight mb-4">
                Your story,<br />
                <span className="bg-gradient-to-r from-indigo-400 via-violet-400 to-pink-400 bg-clip-text text-transparent">beautifully told.</span>
              </h2>
              ...
              <div className="flex flex-col gap-2.5">
                {[
                  { label: "OCEAN Personality AI" },
                  { label: "Vector-powered feed" },
                  { label: "NFT Memory minting" },
                ].map((f, i) => (
                  <motion.div key={f.label} className="flex items-center gap-2.5"
            ...
            <motion.div key="step2-side" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                    ...
                    <div key={key}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <motion.div className={`w-2 h-2 rounded-full ${t.dot}`}
          ...
          {[1, 2].map(s => (
            <div key={s} className="flex items-center gap-2">
              <motion.div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
                animate={step >= s ? { boxShadow: ["0 0 0px rgba(124,58,237,0)", "0 0 12px rgba(124,58,237,0.6)", "0 0 0px rgba(124,58,237,0)"] } : {}}
                transition={{ duration: 2, repeat: Infinity }}
                style={{
              ...
              {Array.from({ length: TOTAL_PAGES }).map((_, i) => (
                <motion.div key={i} className="h-1.5 rounded-full transition-all duration-300"
                  style={{
                    ...
                    boxShadow: i === quizPage ? "0 0 8px rgba(124,58,237,0.6)" : "none",
                  }} />
              ))}
              <span className="text-slate-500 text-xs ml-2">Page {quizPage + 1}/{TOTAL_PAGES}</span>
            </div>
          )}
            ...
            {step === 1 && (
              <motion.div key="step1" className="flex items-center justify-center min-h-full px-6 py-10"
                ...
                <div className="w-full max-w-sm">
                  <div className="mb-7">
                    <h1 className="text-xl font-bold bg-gradient-to-r from-white via-indigo-200 to-violet-300 bg-clip-text text-transparent mb-1">
                      Create your account
                    </h1>
                    <p className="text-slate-500 text-sm">Step 1 of 2 — Basic info</p>
                  </div>
                  ...
                  <form onSubmit={handleStep1} className="space-y-4">
                    ...
                    <motion.button type="submit"
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-white text-sm font-semibold transition-all mt-2 relative overflow-hidden"
                      style={{ background: "linear-gradient(135deg,#4f46e5,#7c3aed)", boxShadow: "0 4px 20px rgba(79,70,229,0.4)" }}
                      whileHover={{ scale: 1.02, boxShadow: "0 4px 32px rgba(124,58,237,0.6)" }} whileTap={{ scale: 0.98 }}>
                      <motion.div
                      ...
                      />
                      <span className="relative flex items-center gap-2">Next: Personality Quiz <ChevronRight className="w-4 h-4" /></span>
                    </motion.button>
                  </form>
                  ...
                  <AnimatePresence mode="wait">
                    <motion.div key={quizPage} className="space-y-4"
                      ...
                      {pageQuestions.map((q, i) => {
                        ...
                        return (
                          <motion.div key={q.id} className="rounded-2xl p-4 transition-all relative overflow-hidden"
                            style={{
                ...
                <div className="flex-shrink-0 px-6 py-4"
                  ...
                  <div className="flex gap-3">
                    <motion.button
                      ...
                      whileHover={{ x: -2, borderColor: "rgba(167,139,250,0.3)" }}
                      whileTap={{ scale: 0.97 }}>
